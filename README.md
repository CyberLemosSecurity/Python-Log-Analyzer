#  Lab: Python Log Analyzer (Infrastructure Monitoring)
...
The environment was structured into two layers to simulate a real data lifecycle:
...
try:
    ...
    print(f"ERRO: Não encontrei o arquivo em: {caminho_log}")y…]()
...
* **Volumetrics:** Monitoring the total number of processed requests to calculate the percentage of error rates.
...
1. Implementation of **Regular Expressions (Regex)** for higher parsing precision.
2. Creation of **Automated Alerts** (via Email or Telegram) when 500 error rates exceed a defined threshold.
